...
prepare_host() {
    ...
    case "${ID:-}" in
        alpine)
            apk add --no-cache util-linux debootstrap parted \
                e2fsprogs e2fsprogs-extra zstd dosfstools ;;
        debian|ubuntu)
            apt-get update -q
            DEBIAN_FRONTEND=noninteractive apt-get install -y \
                util-linux debootstrap parted e2fsprogs zstd dosfstools ;;
        *)
